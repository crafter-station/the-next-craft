...
const clerkLogoPath = path.join(root, "public", "sponsors", "clerk.svg");
  ...
  return Buffer.from(`
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
      ...
      <g transform="translate(70 70)">
        ...
        <text x="91" y="137" fill="${colors.void}" font-family="${pixelFontFamily}" font-size="90" font-weight="700" text-anchor="middle">26</text>
      </g>
      ...
      <g font-family="${pixelFontFamily}" font-weight="700" fill="${colors.text}" letter-spacing="-1" text-anchor="middle" filter="url(#title-shadow)">
        <text x="540" y="458" font-size="79">AGREGA AUTH</text>
        <text x="540" y="555" font-size="78">A TU APP EN</text>
        <text x="540" y="650" font-size="66">MENOS DE 10 MIN</text>
      </g>
  ...
  return Buffer.from(`
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}">
      <filter id="grain" x="0" y="0" width="100%" height="100%">
        <feTurbulence type="fractalNoise" baseFrequency="0.48" numOctaves="3" seed="26" stitchTiles="stitch"/>
        <feColorMatrix type="saturate" values="0"/>
        <feComponentTransfer><feFuncA type="table" tableValues="0 0.36"/></feComponentTransfer>
      </filter>
...
const clerkLogoSvg = (await readFile(clerkLogoPath, "utf8"))
  .replaceAll('fill="white"', `fill="${colors.void}"`)
  .replaceAll('fill-opacity="0.4"', 'fill-opacity="0.55"');
const clerkLogo = await sharp(Buffer.from(clerkLogoSvg))
  .resize({ width: 190 })
  .png()
  .toBuffer();
...
const banner = await sharp(background)
  .composite([
    ...
    { input: clerkLogo, left: 485, top: 952 },
  ])
...
await Promise.all([
  writeFile(path.join(outputDirectory, "clerk-auth-luma-square.png"), banner),
  sharp(banner)
    .webp({ quality: 94, smartSubsample: true })
    .toFile(path.join(outputDirectory, "clerk-auth-luma-square.webp")),
]);

const manifest = JSON.parse(await readFile(manifestPath, "utf8"));
manifest.files = {
  ...manifest.files,
  "events/clerk-auth-luma-square.png": "1080x1080",
  "events/clerk-auth-luma-square.webp": "1080x1080",
};
await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

console.log(
  "Generated public/brand-assets/events/clerk-auth-luma-square.{png,webp}",
);
